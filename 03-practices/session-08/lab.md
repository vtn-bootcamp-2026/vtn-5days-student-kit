...
updated-at: 2026-06-26 06:06 +07:00
---
...
> **Mục tiêu:** Thiết lập thành công kết nối API, tạo **2 Profile Personal Agent** chuyên biệt trên Hermes Desktop Client, tích hợp **tài liệu tri thức cục bộ**, đóng gói **1 Skill quy trình**, thiết lập **1 cấu hình tự động (Cron Job)**, và kiểm thử tính an toàn qua **3 test case bắt buộc**.
...
- **Tích hợp tri thức cục bộ:** Upload tài liệu quy trình lên thư mục **FILES** và chỉ dẫn cho Agent tự tra cứu thay vì copy/paste thủ công.
...
| **Lab 4** | 20 phút | **Tích hợp Tri thức cục bộ (FILES):** Upload tài liệu quy trình vào thư mục FILES và cấu hình SOUL.md. | Agent tra cứu được quy trình tạm ứng hành chính mà không cần copy paste. |
...
### Lab 4 — Tích hợp Tri thức cục bộ (FILES) (20 phút)

Mục tiêu là tích hợp tri thức nghiệp vụ cho Profile trợ lý bằng cách sử dụng thư mục quản lý tệp tin (**FILES**) trên Hermes Dashboard và cấu hình chỉ dẫn truy cập tệp trong `SOUL.md`, giúp Agent tự động phát hiện và đọc tài liệu quy trình thay vì học viên phải dán thủ công dữ liệu mỗi khi hỏi.

#### Các bước thực hiện:
1. **Chuẩn bị và tải lên tài liệu tri thức:**
   - Trong giao diện Hermes Dashboard, nhấp chọn mục **FILES** trên thanh menu bên trái (như hình minh họa).
   - Nhấp chọn **UPLOAD** và tải tệp quy trình nghiệp vụ giả lập [quy-dinh-phuc-loi-va-sla-hanh-chinh.md](synthetic-data/quy-dinh-phuc-loi-va-sla-hanh-chinh.md) lên thư mục làm việc của Hermes.
2. **Khai báo tài liệu trong cấu hình Profile (Chỉ dẫn tra cứu):**
   - Để Agent có thể tự động gọi công cụ đọc file (Tool Call) khi có câu hỏi nghiệp vụ, chúng ta cần khai báo đường dẫn tệp trực tiếp trong tệp định nghĩa ranh giới `SOUL.md` của Profile.
   - Mở tệp `SOUL.md` của Profile `hr_admin_assistant` (nằm tại đường dẫn thư mục cấu hình `$HOME\.hermes\profiles\hr_admin_assistant\SOUL.md`).
   - Cập nhật dòng số 2 để chỉ rõ tệp tài liệu cần tra cứu như sau:
     ```markdown
     2. Tôi giúp Tổ trưởng tổng hợp hiệu quả công việc hành chính và chủ động đọc tệp quy trình `quy-dinh-phuc-loi-va-sla-hanh-chinh.md` trong thư mục FILES để giải đáp các câu hỏi quy chế, tạm ứng.
     ```
   - Nhấp **Save** để lưu lại cấu hình Profile.
3. **Kiểm thử tra cứu tự động (File Tool Test):**
   ...
   - **Kết quả mong đợi (Expected Output):** Agent tự động phát hiện câu hỏi liên quan đến quy chế hành chính, gọi công cụ đọc tệp (File Reader Tool) để truy xuất dữ liệu từ tệp `quy-dinh-phuc-loi-va-sla-hanh-chinh.md` đã upload trong FILES và trả lời chính xác: *Thời gian xử lý tối đa là 4 giờ làm việc cho yêu cầu khẩn cấp (Mức Cao)*.
...
2. **Đánh giá chéo theo Rubric nghiệm thu:**
   ...
   | **FILES Integration** | Không upload tài liệu hoặc trỏ sai đường dẫn. | Có tài liệu nhưng Agent không gọi tool đọc file để truy vấn. | Upload chuẩn xác và chỉ dẫn SOUL.md tốt, Agent truy vấn chính xác quy trình tạm ứng SLA mức Cao. |
...
*   **Giải pháp:** Tắt hoàn toàn cửa sổ PowerShell hiện tại và mở lại cửa sổ mới để nạp lại PATH.
...
*   **Hồ sơ Tác tử (Profile):** Đóng vai trò là một "nhân sự" cụ thể (có danh tính riêng, có tính cách ranh giới quy định bởi `SOUL.md`, và có hòm tài liệu tri thức riêng thông qua thư mục **FILES**). Ví dụ: Profile `hr_admin_assistant` và Profile `hr_recruitment_assistant`.
...
*   **Nhiệm vụ:**
    ...
    3. Học viên sao chép phiếu này, chuyển sang khung chat của `hr_director_assistant` để Agent 2 thẩm định dựa trên quy chế trong thư mục **FILES** và đưa ra quyết định phê duyệt cuối cùng (HITL).
