...
```mermaid
flowchart TD
  Start([Đến lịch hằng tuần]) --> Remind[Tự động gửi thông báo đôn đốc 5 phòng]
  Remind --> Submit[Các phòng ban nhập liệu vào biểu mẫu thống nhất]
  Submit --> TriggerN8N[Workflow n8n tự động trích xuất dữ liệu]
  TriggerN8N --> AICalculate[AI chuẩn hóa dữ liệu & Tự động tính toán số liệu]
  AICalculate --> AIGenReport[AI tự động vẽ biểu đồ & Sinh báo cáo nhân sự]
  AIGenReport --> HITL{HCNS duyệt báo cáo?}
  HITL -->|Chưa đạt| Edit[HCNS chỉnh sửa hoặc yêu cầu gửi lại]
  Edit --> TriggerN8N
  HITL -->|Đạt| AutoSend[Tự động gửi email báo cáo cho Giám đốc]
  AutoSend --> AutoSave[Tự động lưu trữ Cloud theo định dạng chuẩn]
  AutoSave --> End([Kết thúc])
```

## 4. Ảnh render workflow

*(Học viên xuất ảnh sơ đồ từ mermaid.live hoặc render bằng AI rồi chèn vào đây)*

## 5. Sơ đồ so sánh Trước & Sau (Before & After)

*(Học viên chèn ảnh infographic so sánh hiệu quả Trước & Sau khi áp dụng AI)*
