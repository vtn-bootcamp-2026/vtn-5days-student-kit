---
mo-ta: "MẪU skill review-contract (Track A — vibe-working). Đây là cấu trúc kỳ vọng HV build ra ở BT2 sau khi chạy vibe-aiworkforce. Dùng để tham chiếu/đối chiếu, KHÔNG phải bài nộp."
trang-thai: active
phien-ban: v1.0
track: A-sample
created-at: 2026-05-27 07:15 +07:00
updated-at: 2026-06-23 16:15 +07:00
---

# MẪU — Kỹ năng `review-contract` (tham chiếu cho BT2)

> [!NOTE]
> Đây là **skill mẫu** minh họa kết quả kỳ vọng của **BT2** (Track A). HV build skill của mình bằng `vibe-aiworkforce` từ `skill_design.md` — file này chỉ để đối chiếu cấu trúc.
> Trạng thái: **BT2 base** (chỉ `.docx`). Sau BT3 sẽ nhận thêm `.pdf`/`.xlsx`; sau BT4 sẽ có red-flag library mở rộng + condact PII.
