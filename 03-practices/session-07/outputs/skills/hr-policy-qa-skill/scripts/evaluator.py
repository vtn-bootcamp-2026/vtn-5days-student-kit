...
def cross_check_citation(quote: str, source_chunks: list[dict[str, Any]]) -> dict[str, Any]:
    ...
    quote_clean = " ".join(quote.strip().lower().split())

    # Exact match: normalized spaces for robustness
    for chunk in source_chunks:
        content_clean = " ".join(chunk.get("content", "").strip().lower().split())
...
def generate_report(
    ...
    lines: list[str] = [
        f"# Báo cáo Đánh giá RAG Pipeline — HR Policy QA",
        f"",
        f"Thời gian tạo: {now}",
    ...
    # Summary table
    lines.append("## 1. Tổng hợp (Summary)")
    lines.append("")
    ...
    lines.append("## 2. Chi tiết từng câu hỏi (Per-Question Detail)")
    lines.append("")
    ...
    # Detailed breakdown
    lines.append("## 3. Phân tích chi tiết (Detailed Breakdown)")
    lines.append("")

    for i, r in enumerate(results, 1):
        lines.append(f"### Q{i}: {r.get('question', '')}")
        lines.append(f"- **Phân loại**: {r.get('expected_classification', '?')}")
        lines.append(f"- **Kết quả**: {'PASS' if r.get('overall_pass') else 'FAIL'}")
        lines.append(f"- **Đáp án đúng**: {'Có' if r.get('correct_answer') else 'Không'}")
        lines.append(f"- **Có trích dẫn**: {'Có' if r.get('has_citations') else 'Không'}")
        lines.append(f"- **Trích dẫn hợp lệ**: {'Có' if r.get('citations_valid') else 'Không'}")
        lines.append(
        ...
        if r.get("refusal_correct") is not None:
            lines.append(
                f"- **Từ chối đúng**: "
                f"{'Có' if r.get('refusal_correct') else 'Không'}"
            )

        citation_checks = r.get("citation_checks", [])
        if citation_checks:
            lines.append("- **Chi tiết trích dẫn**:")
