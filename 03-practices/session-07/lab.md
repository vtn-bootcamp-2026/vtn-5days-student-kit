...
```bash
python scripts/chunker.py --kb-dir ./kb/hr-policies --output ./kb/chunks.json --chroma
```

`chunker.py` tách theo heading H2, max 500 words, overlap 50 words, tạo 7 trường metadata (chunk_id, doc_id, section, version, status, access_level, word_count). Kỳ vọng 15-20 chunks.

> [!IMPORTANT]
> **Sự khác biệt giữa ChromaDB in-memory và Persistent Client:**
> - **ChromaDB Client mặc định (`chromadb.Client()`):** Dữ liệu lưu trong RAM (in-memory) và sẽ biến mất ngay khi tiến trình kết thúc. Khi bạn chạy lệnh `python retriever.py` độc lập, nó tạo ra một thực thể in-memory mới rỗng, không chứa các vector đã tạo bởi `chunker.py`.
> - **ChromaDB Persistent Client (`chromadb.PersistentClient(path="kb/chroma_db")`):** Dữ liệu vector được lưu trên ổ đĩa trong thư mục `kb/chroma_db`, giúp tiến trình độc lập của `retriever.py` đọc được và thực hiện truy xuất vector (semantic search) thành công qua CLI.
> 
> Do đó, cả `chunker.py` và `retriever.py` cần sử dụng `PersistentClient` và chúng ta chạy `chunker.py` với tham số `--chroma` để nạp dữ liệu vector vào ổ đĩa.
...
### 10.4.1 Cảnh báo tích hợp — Kết nối Persistent ChromaDB thành công qua CLI

Trước đây, khi sử dụng in-memory database (`chromadb.Client()`), dữ liệu lưu trong RAM sẽ biến mất ngay khi tiến trình CLI của `chunker.py` kết thúc. Khi đó, tiến trình CLI độc lập của `retriever.py` khởi tạo một thực thể in-memory mới hoàn toàn rỗng, dẫn đến `vector_search()` luôn rỗng (chỉ BM25 chạy).

Sau khi chuyển đổi sang sử dụng **Persistent Client** (`chromadb.PersistentClient(path="kb/chroma_db")`) và chạy `chunker.py` với tham số `--chroma`, cơ sở dữ liệu vector được ghi trực tiếp lên ổ đĩa. Nhờ đó, tiến trình CLI của `retriever.py` có thể kết nối trực tiếp đến cơ sở dữ liệu ở `kb/chroma_db`, đọc đúng các vector đã nạp, và thực hiện tìm kiếm lai (hybrid search) thực sự thành công qua CLI.
