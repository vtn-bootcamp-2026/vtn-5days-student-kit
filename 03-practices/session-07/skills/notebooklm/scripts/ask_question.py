...
def ask_notebooklm(question: str, notebook_url: str, headless: bool = True) -> str:
    ...
    try:
        ...
        # Capture BASELINE before submit: poll until chat-history element count is
        # STABLE (history fully loaded). Prevents reading a STALE previous answer.
        base_count = -1
        base_last_text = ""
        for _ in range(12):  # up to ~12s
            ...
            try:
                for sel in RESPONSE_SELECTORS:
                    ...
                    if els:
                        cur = len(els)
                        cur_last = els[-1].inner_text().strip()
                        break
            except:
                pass
            if cur == base_count:
                break  # stable
            base_count = cur
            base_last_text = cur_last
            time.sleep(1)
        if base_count < 0:
            base_count = 0
        print(f"  📏 baseline: {base_count} prior response(s)")
        ...
        # Wait for response: ONLY accept an element whose count > base_count
        # (a genuinely new response bubble), then wait for its text to stabilize.
        ...
        last_text = None
        new_seen = False
        deadline = time.time() + 150  # 2.5 minutes timeout
        ...
        while time.time() < deadline:
            ...
            for selector in RESPONSE_SELECTORS:
                try:
                    ...
                    if not elements:
                        continue
                    # Require a NEW response element beyond baseline count
                    if len(elements) <= base_count:
                        continue
                    new_seen = True
                    latest = elements[-1]
        ...
        if not new_seen:
            print("  ⚠️ No new response element detected beyond baseline")
