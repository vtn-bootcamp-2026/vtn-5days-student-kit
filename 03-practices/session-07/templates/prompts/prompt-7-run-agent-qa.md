...
# Prompt 7: Chạy kiểm thử tác nhân hỏi đáp (HR-Policy Agent QA)

Sao chép toàn bộ nội dung trong khung dưới đây và gửi cho trợ lý ảo Antigravity:

```markdown
Chào Antigravity, chúng ta vừa hoàn thành việc tích hợp và cấu hình gói kỹ năng hỏi đáp chính sách tại thư mục `outputs/skills/hr-policy-qa-skill/`.
...
3. **Bước 3: Truy xuất (retrieval):** Đọc các tệp chính sách liên quan trong thư mục `outputs/skills/hr-policy-qa-skill/kb/hr-policies/` (Chính sách nghỉ phép POL-LEAVE-001 và Chính sách thâm niên POL-SENIOR-001).
...
5. **Bước 5: Xuất kết quả:** Trả về kết quả đầu ra khớp chính xác với định dạng JSON schema tại `outputs/skills/hr-policy-qa-skill/schemas/hr-response.schema.json`.
