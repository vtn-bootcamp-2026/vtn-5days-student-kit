...
# Prompt 10: Chạy đánh giá và phân tích lỗi
...
1. Chạy kịch bản đánh giá `evaluator.py` bằng cách truyền bộ câu hỏi kiểm thử từ `03-practice/session-07/synthetic-data/test-questions.csv`, file chunks từ `03-practice/session-07/outputs/skills/hr-policy-qa-skill/kb/chunks.json`, và file câu trả lời mẫu của tác nhân (nếu đã có) hoặc chạy pipeline tự động để sinh câu trả lời:
   ```bash
   python 03-practice/session-07/outputs/skills/hr-policy-qa-skill/scripts/evaluator.py --questions 03-practice/session-07/synthetic-data/test-questions.csv --chunks 03-practice/session-07/outputs/skills/hr-policy-qa-skill/kb/chunks.json --output 03-practice/session-07/outputs/skills/hr-policy-qa-skill/evaluation-report.md
   ```
2. Phân tích tệp báo cáo đánh giá sinh ra tại `03-practice/session-07/outputs/skills/hr-policy-qa-skill/evaluation-report.md` và cho tôi biết:
