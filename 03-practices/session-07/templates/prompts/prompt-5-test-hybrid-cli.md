...
1. **Kiểm thử tìm kiếm ngữ nghĩa (tìm kiếm ngữ nghĩa: semantic search):**
   ```bash
   python 03-practice/session-07/outputs/skills/hr-policy-qa-skill/scripts/retriever.py --query "nghỉ ốm lương như thế nào?" --top-k 3
   ```
...
2. **Kiểm thử tìm kiếm từ khóa chính xác (tìm kiếm từ khóa: keyword search - mã số/tên riêng):**
   ```bash
   python 03-practice/session-07/outputs/skills/hr-policy-qa-skill/scripts/retriever.py --query "POL-LEAVE-001 mục 2.1 ngày phép" --top-k 3
   ```
...
3. **Kiểm thử câu hỏi kết hợp chéo (đối chiếu chéo: cross-reference):**
   ```bash
   python 03-practice/session-07/outputs/skills/hr-policy-qa-skill/scripts/retriever.py --query "thâm niên 6 năm nghỉ phép bao nhiêu ngày" --top-k 3
   ```
